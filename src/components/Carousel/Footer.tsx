...
export default function Footer() {
  return (
    <footer className='font-poppins mt-10 bg-[rgb(24,104,183)] pt-10 text-white'>
      <Container>
        <div className='grid grid-cols-1 gap-x-24 gap-y-10 text-center lg:grid-cols-2 lg:text-left'>
          <div>
            ...
            <div className='mt-4 flex flex-col gap-4 sm:flex-row'>
              ...
              />
              <button className='border-footer-button bg-footer-button flex-none rounded-xl px-8 py-3 font-sans font-medium text-white shadow hover:brightness-110'>
                Sign up
              </button>
            </div>
          </div>
          <div>
            ...
            <div className='mt-4 inline-flex gap-x-2'>
              {['twitter', 'instagram', 'reddit', 'youtube', 'email'].map(
                (social) => (
                  <SocialIcon
                    key={social}
                    url='#'
                    network={social}
                    className='[&_svg]:border-footer-button [&_svg]:!rounded-xl [&_svg]:border [&_svg]:hover:brightness-110'
                    fgColor='#fff'
        ...
        <div className='border-t border-white/20 py-8'>
          <p className='text-center text-xs font-light'>
            Cloned by{' '}
            <a
              href='https://heyley.dev'
              target='_blank'
              className='font-normal underline decoration-teal-500'
            >
              heyley.dev
            </a>
          </p>
