...
export default function HeroSection() {
  ...
  return (
    <div className="relative h-full">
      ...
      />
      <div className="relative z-10 -mt-[4.25rem] pt-[4.25rem] h-full">
        <Container className="px-0 sm:px-4 h-full">
        ...
        <div className="swiper-wrapper">
          {CAROUSEL_ITEMS.map((item, i) => (
            <SwiperSlide key={i} className="swiper-slide">
              <div
                className="group relative flex aspect-square cursor-pointer overflow-hidden rounded-2xl bg-white/10"
              >
                <img
                  src={`/carousel/${item.image}`}
                  loading="lazy"
                  className="pointer-events-none absolute inset-0 w-full h-full object-cover object-center transition-transform duration-500 will-change-transform group-hover:scale-110"
                  alt={item.name}
                />
                <div className="pointer-events-none absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 text-white">
